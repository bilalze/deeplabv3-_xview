...
from dataloader_mine2 import creator
...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='voc',
                        choices=['voc', 'cityscapes','xview'], help='Name of dataset')
...
def get_dataset(opts):
    ...
    if opts.dataset=='xview':
        train_transform = et.ExtCompose([
            #et.ExtResize(size=opts.crop_size),
            et.ExtRandomScale((0.8, 2.0)),
            et.ExtRandomCrop(size=(opts.crop_size, opts.crop_size), pad_if_needed=True),
            et.ExtRandomHorizontalFlip(),
            et.ExtToTensor(),
            et.ExtNormalize(mean=[0.485, 0.456, 0.406],
...
def validate( model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, (images, labels,labels2) in tqdm(enumerate(loader)):
            if i>37:
                break       
...
def main():
    ...
    val_loader = data.DataLoader(
        val_dst, batch_size=opts.val_batch_size, shuffle=False)
